...
int main(int argc, char* argv[]) {
    ...
    double length = 14;
    double width = 4;
    /////////// option 1: load bump.obj directly which has z pointing up /////////////////
//    std::string terrain_file = GetChronoDataFile("vehicle/terrain/meshes/bump.obj");
//    terrain.SetPlane(ChCoordsys<>(ChVector<>(0, 0, -3.5)));


    /// option 2: load bump_YUP.obj as a triagnular mesh, and rotate so the Z frame is pointing up /////
    //// write the mesh in a file directory, and load it again as terrain ///// 
      std::string terrain_file = GetChronoDataFile("vehicle/terrain/meshes/bump_YUP.obj");
      // load mesh from obj file 
      auto trimesh = geometry::ChTriangleMeshConnected::CreateFromWavefrontFile(terrain_file, false, false);
      ChMatrix33<double> rot(Q_ROTATE_Y_TO_Z);  // create rotation matrix so the mesh is Z up
      trimesh->Transform(VNULL, rot); // perform transformation on the mesh
      geometry::ChTriangleMeshConnected::WriteWavefront("test.obj", {*trimesh}); // write the transformed mesh to the directory
      terrain.SetPlane(ChCoordsys<>(ChVector<>(0, 0, -4)));
      terrain_file = "C:/Users/fang/source/repos/projectchrono/chrono_build/bin/Release/test.obj"; // load the mesh with Z up

      // ChronoTerrain5.obj//
    /* std::string terrain_file = GetChronoDataFile("vehicle/terrain/meshes/ChronoTerrain5.obj");    
      auto trimesh = geometry::ChTriangleMeshConnected::CreateFromWavefrontFile(terrain_file, false, false);
      ChMatrix33<double> rot(Q_ROTATE_Y_TO_Z);
      trimesh->Transform(ChVector<>(0, 0, -325), rot);
      geometry::ChTriangleMeshConnected::WriteWavefront("test.obj", {*trimesh});
      terrain.SetPlane(ChCoordsys<>(ChVector<double>(0, 0, -2), QUNIT));
      terrain_file = "C:/Users/fang/source/repos/projectchrono/chrono_build/bin/Release/test.obj";
*/
      terrain.Initialize(terrain_file, mesh_resolution * 10);
    ...
    switch (vis_type) {
        case ChVisualSystem::Type::IRRLICHT: {
            ...
            vis_irr->AddLogo();
            vis_irr->AddSkyBox();
            vis_irr->AddCamera(ChVector<>(5, 5, 3), ChVector<>(0, 0, wheel_range));
